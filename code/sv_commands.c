...
void SV_Version() {
	...
	#ifdef xDEBUG
	printf("Call of Duty Extended Developer build %d by riicchhaarrd\nA lot of thanks to kungfooman and libcod\n", CURRENTBUILD);
	#else
	printf("Call of Duty Extended build %d by riicchhaarrd\nA lot of thanks to kungfooman and libcod\n", CURRENTBUILD);
	#endif
}
...
void SV_XStatus_f() {
	...
	Com_Printf( "num guid   score ping name            lastmsg address               qport rate\n" );
	Com_Printf( "--- ------ ----- ---- --------------- ------- --------------------- ----- -----\n" );
	...
	for(i = 0; i < sv_maxclients->integer; i++) {
		...
		Com_Printf( "%3i ", i );
		Com_Printf( "%6i ", guid);
		Com_Printf( "%5i ", SV_GetClientScore(cl));
		...
		s = NET_AdrToString( cl->remoteAddress );
		Com_Printf( "%s", s );
		l = 22 - strlen( s );
		for ( j = 0 ; j < l ; j++ )
			Com_Printf( " " );
	
		Com_Printf( " %5i", cl->qport );
...
void SV_Ban_f( void ) {
	...
	if(Cmd_Argc()!=4) {
		Com_Printf("Usage: %s <substring> \"reason\" \"guid|ip\"\n", Cmd_Argv(0));
		return;
	}
...
void SV_UnbanGUID_f() {
	...
	FILE* f = fopen("guidban.txt", "w");
	
	if(!f) {
		Com_Printf("Failed to open guidban.txt\n");
		return;
	}
	...
	int g;
	std::string line;
	std::ifstream guid2("guidban.txt");
	if(guid2.is_open()) {
		bannedGUIDs.clear();
		while(std::getline(guid2, line)) {
			if(line.empty())
				continue;
			g = atoi(line.c_str());
			
			if(!strcmp(line.c_str(), Cmd_Argv(1))) {
				found = qtrue;
				Com_Printf("GUID '%d' has been unbanned.\n", guid);
				continue;
			}
			fprintf(f, "%d\n", g);
			bannedGUIDs.push_back( g );
		}
		if(!found) {
			Com_Printf("GUID '%d' was not banned.\n", guid);
		}
	} else {
		Com_Printf("Error whilst reading guidban.txt, failed to correctly unban.\n");
	}
